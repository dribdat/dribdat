...
def get_users_by_search(search_by, MAX_COUNT=200):
    ...
    if search_by.startswith("*"):
        ...
        users = users.filter(or_(User._my_skills.ilike(q), User._my_wishes.ilike(q)))
...
def get_dribs_paginated(page=1, per_page=10, host_url=""):
    ...
    latest_dribs = Activity.query.filter(
        or_(Activity.action == "post", Activity.name == "boost")
    ).order_by(Activity.id.desc())
    dribs = latest_dribs.paginate(page=page, per_page=per_page)
    dribs.items = [d for d in dribs.items if not d.project.is_hidden and d.content]
...
def get_contributor_stats(max_users=25):
    ...
    results = (
        db.session.query(
            stmt.c.username, stmt.c.total_activities, stmt.c.commits, stmt.c.comments
        )
        .order_by(stmt.c.total_activities.desc())
        .all()
    )
