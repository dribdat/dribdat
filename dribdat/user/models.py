...
class User(UserMixin, PkModel):
    ...
    def joined_projects(self, with_challenges=True, limit=-1, event=None):
        ...
        project_ids = []
        for a in activities:
            if limit > 0 and len(projects) >= limit:
                break
                    ...
                    vvlist[vtype].append(
                        {
                            ...
                            "name": get_any_key(
                                vv, ["name", "institution", "organization", "language"]
                            ),
                            "summary": get_any_key(
                                vv,
                                [
    ...
    def posted_challenges(self):
        ...
        projects = (
            Project.query.filter_by(user_id=self.id, progress=0, is_hidden=False)
    ...
    def activity_count(self):
        ...
        return Activity.query.filter_by(user_id=self.id).count()
    ...
    def check_hashword(self, value):
        ...
        timediff = datetime.now(UTC).replace(tzinfo=None) - self.updated_at.replace(
            tzinfo=None
        )
...
class Event(PkModel):
    ...
    )  # default hashtags for social media
    ...
    )  # a longer text about the event
    instruction = Column(
        db.UnicodeText(), nullable=True
    )  # tips for logged-in event participants
    boilerplate = Column(
        db.UnicodeText(), nullable=True
    )  # tips to show on starting a new project
    aftersubmit = Column(
        db.UnicodeText(), nullable=True
    )  # additional content to show new projects
    ...
    community_url = Column(
        db.String(255), nullable=True
    )  # user forum or social media link

    custom_css = Column(
        db.UnicodeText(), nullable=True
    )  # styles to customize frontend with CSS

    status = Column(db.UnicodeText(), nullable=True)  # used for org announcements
    community_embed = Column(
        db.UnicodeText(), nullable=True
    )  # code of conduct in footer
    certificate_path = Column(
        db.String(1024), nullable=True
    )  # URL to certificate download
    ...
    def get_ical(self, host_url=""):
        ...
        if host_url:
            descriptives.append(host_url)
        location = self.location or self.hostname
        icalev = iCalEvent()
        icalev.add("CREATED", datetime.today())
        icalev.add("DESCRIPTION", "\n\n".join(descriptives))
        if location:
            icalev.add("LOCATION", location)
        if self.name:
            icalev.add("SUMMARY", self.name)
        if self.starts_at:
            icalev.add("DTSTART", self.starts_at)
        if self.ends_at:
            icalev.add("DTEND", self.ends_at)
        ics = iCalendar()
...
class Project(PkModel):
    ...
    def team_count(self):
        ...
        return Activity.query.filter_by(project_id=self.id, name="star").count()
    ...
    def get_team(self, with_spectators=False):
        ...
        if with_spectators:
            ...
            activities = Activity.query.filter_by(project_id=self.id, name="star").all()
        members = []
        if self.user:
    ...
    def get_stats(self):
        ...
        for act in q:
            ...
            if self.event:
                if (
                    act.timestamp > self.event.starts_at
                    and act.timestamp < self.event.ends_at
                ):
                    s_during += 1
...
class Activity(PkModel):
    ...
    name = Column(
        db.Enum("review", "boost", "create", "update", "star", name="activity_type")
    )
