...
def index():
    ...
    if current_event is None:
        ...
        sum_hidden = (
            Project.query.filter(Project.event_id == current_event.id)
            .filter(Project.is_hidden)
            .count()
        )
    stats = [
        {
        ...
        {"value": User.query.count(), "text": "Users", "icon": "user", "height": 7},
        {
    ...
    if sum_hidden > 0:
        msgs.append(
            "%d hidden projects in the featured event " % sum_hidden
            + " may need moderation."
        )
...
def users(page=1):
    ...
    if sort_by == "id":
        ...
        users = User.query.order_by(User.is_admin.desc())
    elif sort_by == "sso":
        users = User.query.order_by(User.sso_id.desc())
    elif sort_by == "updated":
        users = User.query.order_by(User.updated_at.desc())
    elif sort_by == "email":
...
def events():
    ...
    return render_template(
        "admin/events.html", data=eventpages, endpoint="admin.events", active="events"
    )
...
@admin_required
def projects(page=1):
    projects = Project.query.order_by(Project.updated_at.desc())
    search_by = request.args.get("search")
    if search_by and len(search_by) > 1:
        q = "%%%s%%" % search_by.lower()
...
def category_projects(category_id):
    ...
    projects = Project.query.filter_by(category_id=category_id).order_by(
        Project.id.desc()
    )
    return render_template(
        "admin/projects.html",
...
def event_projects(event_id):
    ...
    return render_template(
        "admin/projects.html", projects=projects, event=event, active="projects"
    )
...
def project_new():
    ...
    form.event_id.choices = [
        (e.id, e.name) for e in Event.query.order_by(Event.id.desc())
    ]
    current_event = Event.query.filter_by(is_current=True).first()
...
def categories():
    ...
    return render_template(
        "admin/categories.html", categories=categories, active="categories"
    )
...
def role(role_id):
    ...
    return render_template(
        "admin/role.html", role=role, form=form, users=role.users[:50]
    )
...
def teambuilder_match():
    ...
    if matches is None:
        flash(
            "Could not find an optimal solution. Try adjusting constraints.", "danger"
        )
        return redirect(url_for("admin.teambuilder"))

    return render_template(
        "admin/teambuilder_results.html",
        event=current_event,
        matches=matches,
        active="teambuilder",
    )
